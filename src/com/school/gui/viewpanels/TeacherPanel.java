...
public class TeacherPanel extends JPanel
{
	...
	public JPanel createTeacherPanel()
	{
		...
		teacherTable.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			...
			{
				selectTeacher.setUserid(teacherTable.getValueAt(teacherTable.getSelectedRow(), 0).toString());
				selectTeacher.setId(Integer.parseInt(teacherTable.getValueAt(teacherTable.getSelectedRow(), 0).toString().substring(1)));
			}
		});
		bEdit.addActionListener(new ActionListener()
		{
			...
			{
				EditTeacher_IF editTeacher = new EditTeacher_IF();
				EditTeacher_IF.teacher = new Teacher(selectTeacher.getUserid(), selectTeacher.getId(), UserType.TEACHER);
				EditTeacher_IF.id = selectTeacher.getId();
				
				MainWindowSetup.JDPsystemSetup.add(editTeacher.editTeacher_IF(), JLayeredPane.POPUP_LAYER);
			}
		});
